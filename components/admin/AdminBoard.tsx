...
export type AdminModuleView = "list" | "board" | "calendar";

const MODULE_VIEWS: readonly AdminModuleView[] = ["list", "board", "calendar"];
...
function isModuleView(value: unknown): value is AdminModuleView {
  return typeof value === "string" && (MODULE_VIEWS as readonly string[]).includes(value);
}
 ...
 * Un `?vista=` en la URL manda sobre lo recordado (issue #56: la ruta vieja
 * `/calendario` entra a `/eventos?vista=calendario`).
 */
export function useAdminModuleView(module: string): [AdminModuleView, (view: AdminModuleView) => void] {
  ...
  useEffect(() => {
    try {
      ...
      if (isModuleView(stored)) setView(stored);
    } catch {
...
const VIEW_LABELS: Record<AdminModuleView, { label: string; icon: "menu" | "overview" | "calendar" }> = {
 ...
 * lista y tablero; los módulos que además tienen calendario suman esa vista
 * (issue #56).
 */
export function AdminViewSwitch({
  ...
  return (
    <div className="admin-viewswitch" role="group" aria-label={label}>
      {views.map((value) => (
        <button key={value} type="button" className="admin-viewswitch-btn" aria-pressed={view === value} onClick={() => onChange(value)}>
          <AdminIcon name={VIEW_LABELS[value].icon} size={14} />
          {VIEW_LABELS[value].label}
        </button>
      ))}
    </div>
  );
