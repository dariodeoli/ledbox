...
const TONES: Record<string, AdminTone> = {
  // Estados con color (issue #75, 26-09-2026): el color de estado se dice con
  // tres tonos —verde `ok` (aprobado / confirmado / cobrado), naranja `warn`
  // (pendiente / en curso) y rojo `danger` (esperando aprobación / vencido /
  // rechazado)—. El texto del estado no cambia y `accent`/`info`/`neutral`
  // quedan para lo que no es estado (roles, tipos, categorías, IVA).
  DRAFT: "neutral",
  NEW: "warn",
  CONTACTED: "warn",
  QUOTED: "warn",
