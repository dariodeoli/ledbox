...
export async function GET() {
  if (!(await requireAdminRole(["OWNER", "ADMIN", "FINANCE", "VIEWER"]))) return jsonError("Unauthorized", 401);
...
export async function POST(request: Request) {
  if (!(await requireAdminRole(["OWNER", "ADMIN", "FINANCE"]))) return jsonError("Unauthorized", 401);
